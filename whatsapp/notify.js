...
export async function notifyOrderConfirmation(phone, { id, produce, qty, seller }) {
  const msg =
    `✅ *mARI Platform by Pameltex Tech Order Confirmation*\n\n` +
    `Your enquiry for *${qty} of ${produce}* has been sent to *${seller || 'the seller'}*.\n\n` +
    `📋 Ref: ${id}\n` +
    `🌐 Track on web: ${WEBAPP_URL}\n\n` +
    `Reply *MENU* to return to the mARI Platform by Pameltex Tech bot.`;
...
export async function notifyCreditApplication(phone, { amount, currency = '' }) {
  const msg =
    `💳 *mARI Platform by Pameltex Tech Credit Application*\n\n` +
...
export async function notifyCreditApproved(phone, { amount, currency = '', rate }) {
  const msg =
    `🎉 *Credit Approved!*\n\n` +
    `Your mARI Platform by Pameltex Tech micro-credit of *${currency}${amount}* at *${rate}% p.a.* has been approved.\n\n` +
