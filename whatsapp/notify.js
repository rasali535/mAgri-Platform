...
export async function notifyCreditApplication(phone, { amount, currency = '' }) {
  const msg =
    `💳 *mAgri-Platform Credit Application*\n\n` +
...
export async function notifyCreditApproved(phone, { amount, currency = '', rate }) {
  const msg =
    `🎉 *Credit Approved!*\n\n` +
    `Your mAgri-Platform micro-credit of *${currency}${amount}* at *${rate}% p.a.* has been approved.\n\n` +
